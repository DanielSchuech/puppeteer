...
class NetworkManager extends EventEmitter {
    ...
    if (event.redirectUrl) {
      ...
      if (request) {
        this._handleRequestRedirect(
          ...
          false /* fromDiskCache */,
          false /* fromServiceWorker */,
          null /* securityDetails */
        );
        this._handleRequestStart(
          request._requestId,
  ...
  _onRequestWillBeSent(event) {
    if (this._protocolRequestInterceptionEnabled) {
      ...
      const request = interceptionId
        ? this._interceptionIdToRequest.get(interceptionId)
        : null;
      if (request) {
    ...
    if (event.redirectResponse) {
      ...
      if (request)
        this._handleRequestRedirect(
          ...
          event.redirectResponse.fromServiceWorker,
          event.redirectResponse.securityDetails
        );
    }
    this._handleRequestStart(
      event.requestId,
  ...
  _onResponseReceived(event) {
    ...
    const response = new Response(
      ...
      event.response.securityDetails
    );
    request._response = response;
    this.emit(NetworkManager.Events.Response, response);
  }
